...
# -----------------------------------------------------------------------------
# Função para gerar um data.frame com k grupos a partir de intervalos fornecidos
gerar_dados <- function(nGrp, intAmos, intMed, intDes){

	# Verificando se os argumentos são válidos
	if(!is.numeric(nGrp) || length(nGrp) != 1) stop("Erro: 'nGrp' deve ser numérico e de tamanho 1")
	if(!is.numeric(intAmos) || length(intAmos) != 2) stop("Erro: 'intAmos' deve ser numérico e de tamanho 2")
	if(!is.numeric(intMed) || length(intMed) != 2) stop("Erro: 'intMed' deve ser numérico e de tamanho 2")
	if(!is.numeric(intDes) || length(intDes) != 2) stop("Erro: 'intDes' deve ser numérico e de tamanho 2")
...
resEst <- function(vetor){
	...
	if(!is.numeric(vetor) || length(vetor) < 2) stop("Erro: O argumento de entrada deve ser um vetor numérico e conter mais de um elemento")
...
resEstSim <- function(data){
	...
	# Verifica se o objeto é um data.frame e contém pelo menos duas colunas
	if(!is.data.frame(data) || ncol(data) < 2 || nrow(data) < 2) stop("Erro: O argumento de entrada deve ser um data.frame com pelo menos duas colunas e conter mais de uma linha")
...
remOutGrup <- function(dados, indices_fatores, indice_resposta){
	...
	if(!is.data.frame(dados)) stop("Erro: 'dados' deve ser um objeto do tipo 'data.frame'")
	if(!is.numeric(indices_fatores) || !is.numeric(indice_resposta)) stop("Erro: 'indices_fatores' e 'indice_resposta' devem ser numéricos")
	if(any(indices_fatores > ncol(dados)) || indice_resposta > ncol(dados)) stop("Erro: Índices de colunas são inválidos")

	# Inicializa a contagem de outliers
	contagem_outliers <- matrix(0, nrow = nrow(dados), ncol = length(indices_fatores))

	# Itera sobre cada coluna de fator
	for(indice_fator in indices_fatores){
		niveis_fator <- levels(as.factor(dados[[indice_fator]]))

		# Para cada nível, identifica outliers na coluna de resposta
	...
	# Filtra e retorna o dataframe sem os outliers
	dados_sem_outliers <- dados[rowSums(contagem_outliers) == 0, ]
...
mult_lim <- function(vetor, mult = c(2, 3, 5, 10)){
	...
	# --- Verificações de entrada ---
	if(missing(vetor)) stop("Erro: vetor ausente")
	if(!is.numeric(vetor)) stop("Erro: vetor deve ser numérico")
...
ks_teste <- function(VarE, show = FALSE){
	...
	# Verificações de entrada
	if(!is.numeric(VarE) || length(VarE) < 2) stop("Erro: O argumento de entrada deve ser um vetor numérico e conter mais de um elemento")

	# Teste de Kolmogorov-Smirnov
	VarE_jittered <- jitter(VarE)
	teste_KS <- ks.test(VarE_jittered, "pnorm", mean = mean(VarE), sd = sd(VarE))
...
graf_apre <- function(dados, limite_x, limite_y, rotulo_x, pos_legenda1, pos_legenda2){

	# Verificações de entrada
	if (missing(dados)) stop("Erro: forneça 'dados'")
	if(!is.numeric(dados) || length(dados) < 2) stop("Erro: 'dados' deve ser numérico e conter pelo menos 2 observações")
	if(missing(limite_x) || !is.numeric(limite_x) || length(limite_x) != 2) stop("Erro: forneça 'limite_x' como vetor numérico de tamanho 2")
	if(limite_x[1] >= limite_x[2]) stop("Erro: 'limite_x[1]' deve ser menor que 'limite_x[2]'")
	if(missing(limite_y) || !is.numeric(limite_y) || length(limite_y) != 1 || limite_y <= 0) stop("Erro: 'limite_y' deve ser um valor numérico positivo")
	if(missing(rotulo_x) || !is.character(rotulo_x) || length(rotulo_x) != 1) stop("Erro: 'rotulo_x' deve ser uma string")
	# Posições das legendas
	if(missing(pos_legenda1) || missing(pos_legenda2)) stop("Erro: posições das legendas não foram informadas")
	# Valida posições
	validar_posicao <- function(pos) {
		if (is.character(pos)) return(TRUE)
		if (is.numeric(pos) && length(pos) == 2) return(TRUE)
		stop("Erro: posição de legenda deve ser texto ou coordenadas numéricas c(x, y)")
	}
	...
	if(largura_cls <= 0) stop("Erro: não foi possível calcular classes do histograma (dados constantes?)")
	...
	op <- par(mar = c(3, 3.2, 1, 1))
		...
		hist(dados, prob = TRUE,
			xlim = limite_x, ylim = c(0, limite_y), breaks = breaks_hist,
			right = FALSE, xlab = rotulo_x, ylab = "",
			main = "", border = 0, col = "lightgrey",
			font.lab = 2, font.axis = 2, las = 1, cex.axis = 1,
			xaxs = "i", yaxs = "i", bty = "n"
		)
...
# -----------------------------------------------------------------------------
# Função para apresentação de gráficos limpos
graf_limp <- function(){
...
graf_resid <- function(y_pred, residuos, x.lim, y.lim, x.lim.mult, y.lim.mult, x.lab, y.lab, texto, pos.texto){
	...
	# --- Verificações de entrada ---
	if(missing(y_pred) || missing(residuos)) stop("Erro: forneça 'y_pred' e 'residuos'")
	if(length(y_pred) != length(residuos)) stop("Erro: 'y_pred' e 'residuos' devem ter o mesmo tamanho")
	if(!is.numeric(y_pred) || !is.numeric(residuos)) stop("Erro: 'y_pred' e 'residuos' devem ser numéricos")
	if(!missing(x.lim) && !missing(x.lim.mult)) stop("Erro: usar somente 'x.lim' ou 'x.lim.mult' para limite em x")
	if(!missing(y.lim) && !missing(y.lim.mult)) stop("Erro: usar somente 'y.lim' ou 'y.lim.mult' para limite em y")
	...
	# Limites eixo x
	if(!missing(x.lim)){
		if(!is.numeric(x.lim) || length(x.lim) != 2) stop("Erro: 'x.lim' deve ser um vetor numérico de tamanho 2")
	...
	# Limites eixo y
	if(!missing(y.lim)){
		if(!is.numeric(y.lim) || length(y.lim) != 2) stop("Erro: 'y.lim' deve ser um vetor numérico de tamanho 2")
		if(y.lim[1] >= y.lim[2]) stop("Erro: 'y.lim[1]' deve ser menor que 'y.lim[2]'")

		va <- range(residuos, na.rm = TRUE)
		if(y.lim[1] > va[1] || y.lim[2] < va[2]) stop("Erro: 'y.lim' deve cobrir todo o intervalo de 'residuos'")
	...
	if(missing(x.lab)) x.lab <- expression(bold(hat(y)))
	if(missing(y.lab)) y.lab <- expression(bold(epsilon))
	if(!(is.expression(x.lab) || (is.character(x.lab) && length(x.lab) == 1))) stop("Erro: 'x.lab' deve ser expression ou caractere unitário")
	if(!(is.expression(y.lab) || (is.character(y.lab) && length(y.lab) == 1))) stop("Erro: 'y.lab' deve ser expression ou caractere unitário")
	if(!missing(texto)){
	...
	op <- par(mar = c(5, 5, 1, 1), mgp = c(3, 0.8, 0), las = 1)
		...
		if(!missing(texto)) legend(pos.texto, legend = texto, bty = "n")
...
# -----------------------------------------------------------------------------
# Função para montar uma equação linear a partir de um modelo
